...
const char * const NESTCOIN_CONF_FILENAME = "nestcoin.conf";
const char * const NESTCOIN_PID_FILENAME = "nestcoin.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "nestcoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Nestcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Nestcoin
    // Mac: ~/Library/Application Support/Nestcoin
    // Unix: ~/.nestcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Nestcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Nestcoin";
#else
    // Unix
    return pathRet / ".nestcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", NESTCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", NESTCOIN_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Nestcoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Nestcoin Core") == std::string::npos) {
