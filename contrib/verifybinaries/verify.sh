...
###   nestcoincore.org and nestcoin.org and compares them.
...
WORKINGDIR="/tmp/nestcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://nestcoincore.org"
HOST2="https://nestcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="nestcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'nestcoin-',
...
if [ $? -ne 0 ]; then
   echo "nestcoin.org failed to provide signature file, but nestcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "nestcoin.org and nestcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Nestcoin Core binary release signing key installed?"
   fi
